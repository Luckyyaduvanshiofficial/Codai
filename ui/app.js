        ...
        try {
            const response = await fetch(`${State.backendUrl}/v1/chat/completions`, {
                ...
                body: JSON.stringify({
                    ...
                    stream: true,
                    // Tiny CPU models (Qwen3/3.5) can burn the entire token
                    // budget thinking and never answer. Ask for the direct
                    // response; advanced users can hit the engine API
                    // directly with enable_thinking for reasoning mode.
