...
class CodaiProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _forward_to_engine(
        ...
        # CPU generation of long answers takes minutes; only the connection
        # setup needs a short timeout. Streaming reads still fail fast if the
        # engine dies because each socket read enforces the same timeout.
        is_chat = self.path.split("?", 1)[0] == "/v1/chat/completions"
        forward_timeout = 300 if is_chat else 30
...
class CodaiProxyServer(http.server.ThreadingHTTPServer):
    # Reuse the address so restarts right after a shutdown don't hit TIME_WAIT
